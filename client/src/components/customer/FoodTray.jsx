    ...
    const toggleItemSelection = (cart_item, id) => {
        setCartItems((prevCartItems) =>
            prevCartItems.map((item) => {
                if (item.id === id) {
                    const itemTotal = Math.round((item.itemPrice - ((item.itemPrice * item.discount) / 100)) * item.quantity);
                    const itemDiscount = Math.round(((item.itemPrice * item.discount) / 100) * item.quantity);
                ...
                if (item.id === itemId) {
                    ...
                    if (item.selected) {
                        setTotalPrice((prevTotalPrice) =>
                            prevTotalPrice - Math.round(diffQuantity * (item.itemPrice - ((item.itemPrice * item.discount) / 100)))
                        );
                        setTotalDiscount((prevTotalDiscount) =>
                            prevTotalDiscount - Math.round(diffQuantity * ((item.itemPrice * item.discount) / 100))
                        );
                    }
        ...
        setCartItems((prevCartItems) =>
            prevCartItems.map((item) => {
                if (item.id === itemId) {
                    ...
                    if (item.selected) {
                        setTotalPrice((prevTotalPrice) =>
                            prevTotalPrice + Math.round(item.itemPrice - ((item.itemPrice * item.discount) / 100))
                        );
                        setTotalDiscount((prevTotalDiscount) =>
                            prevTotalDiscount + Math.round((item.itemPrice * item.discount) / 100)
                        );
                    }
                ...
                <div className="container mx-auto max-w-4xl bg-white p-6 rounded-lg shadow-lg">
                    ...
                    <div className="flex justify-center">
                        <button
                            ...
                            className={`font-bold py-3 px-10 rounded-full
                                ${disablePlaceOrder || !formattedAddress || totalPrice === 0
                                    ? "bg-gray-300 text-gray-500 cursor-not-allowed"
                                    : "bg-yellow-500 hover:bg-yellow-600 text-white  shadow-lg transition-transform hover:scale-105"
                                }`}
                            disabled={!formattedAddress || totalPrice === 0 || disablePlaceOrder}
                        >
                            Place Order
                        </button>
                    </div>
